...
import sy.iyad.idlib.Ready.ApiException;
import sy.iyad.idlib.Ready.MikrotikApiException;
import sy.iyad.idlib.Ready.ResultListener;
...
/**
    ...
    public final class ApiImp extends Api {
         ...
         * problem connecting
         */
        public static Api connect(SocketFactory fact, String host, int port, int timeOut) throws ApiException {
            ApiImp con = new ApiImp();
        ...
        public void login(String username, String password) throws MikrotikApiException {
            if (username.trim().isEmpty()) {
                throw new ApiException("API username cannot be empty");
            }
        ...
        public void close() throws ApiException {
            if (!connected) {
                throw new ApiException(("Not/no longer connected to remote Mikrotik"));
            }
            connected = false;
            processor.interrupt();
            reader.interrupt();
            try {
                ...
                throw new ApiException(String.format("Error closing socket: %s", ex.getMessage()), ex);
            }
        }
    ...
    private void execute(Command cmd, ResultListener lis) throws MikrotikApiException {
         ...
         */
        private void open(String host, int port, SocketFactory fact, int conTimeout) throws ApiException {
            try {
                ...
                connected = false;
                throw new ApiException(String.format("Unknown host '%s'", host), ex);
            } catch (IOException ex) {
                connected = false;
                throw new ApiException(String.format("Error connecting to %s:%d : %s", host, port, ex.getMessage()), ex);
            }
        }
    ...
    private final Map<String, ResultListener> listeners;
        ...
        private int timeout = Api.DEFAULT_COMMAND_TIMEOUT;
        ...
        private class Reader extends Thread {
            ...
            private String take() throws ApiException, ApiDataException {
                Object val = null;
                try {
                    val = queue.take();
                } catch (InterruptedException ex) {
                    throw new ApiException("Interrupted while reading data from queue.", ex);
                }
                if (val instanceof ApiException) {
                    throw (ApiException) val;
        ...
        private class Processor extends Thread {
            ...
            private Result unpackRe() throws ApiDataException, ApiException {
                nextLine();
                int l = 0;
            ...
            private String unpackResult(String first) throws ApiException, ApiDataException {
                    ...
                    synchronized (this) { // don't wait if we already have a result.
                        ...
                        while (!complete && (waitTime > 0)) {
                            ...
                            if ((waitTime <= 0) && !complete) {
                                err = new ApiException(String.format("Command timed out after %d ms", timeout));
                            }
                        }
                    }
                } catch (InterruptedException ex) {
